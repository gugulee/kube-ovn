...
func (c *Controller) checkAttachNetwork(svc *corev1.Service) (string, string, error) {
	var attachmentName, attachmentNs string

	providerName := getAttachNetworkProvider(svc)
	values := strings.Split(providerName, ".")
	if len(values) <= 1 {
		return attachmentName, attachmentNs, fmt.Errorf("The provider name should be consisted of name and namesapce")
	}
	attachmentName = values[0]
	attachmentNs = values[1]

	networkClient := c.config.AttachNetClient.K8sCniCncfIoV1().NetworkAttachmentDefinitions(attachmentNs)
	_, err := networkClient.Get(context.Background(), attachmentName, metav1.GetOptions{})
	if err != nil {
		klog.Errorf("failed to get network attachment definition %s in namespace %s, err: %v", attachmentName, attachmentNs, err)
		return attachmentName, attachmentNs, err
	}

	return attachmentName, attachmentNs, nil
}
...
func (c *Controller) genLbSvcDeployment(svc *corev1.Service) (dp *v1.Deployment) {
	...
	attachmentName, attachmentNs, _ := c.checkAttachNetwork(svc)
	providerName := getAttachNetworkProvider(svc)
...
func (c *Controller) createLbSvcPod(svc *corev1.Service) error {
	...
	if needToCreate {
		...
		_, err := c.config.KubeClient.AppsV1().Deployments(svc.Namespace).Update(context.Background(), deploy, metav1.UpdateOptions{})
		if err != nil {
